...
\begin{document}
	...
	\section{Introduzione}
		\subsection{Scopo del documento}
			...
			migliorandone l'efficacia\ped{\emph{G}}, riducendo il numero di errori e i tempi di sviluppo.
		...
		\subsection{Riferimenti}
			\subsubsection{Informativi}
				\begin{itemize}
					\item Specifiche UTF-8\ped{\emph{G}}: \\ \url{http://unicode.org/faq/utf_bom.html}
					\item Licenza MIT: \\ \url{https://opensource.org/licenses/MIT}
					\item Scala\ped{\emph{G}} Programming Language: \\ \url{http://www.scala-lang.org/}
					\item ISO/IEC 12207: \\ \url{http://www.iso.org/iso/catalogue_detail?csnumber=43447}
					\item ISO 8601:2004\ped{\emph{G}}: \\ \url{http://www.iso.org/iso/home/standards/iso8601.htm}
	...
	\begin{table}[H]
		...
		\subsubsection{Tecniche di analisi e classificazione requisiti}
			...
			da eventuali Casi d'uso\ped{\emph{G}} emersi da Brainstorming\ped{\emph{G}} o riunioni con il committente.
	
			I requisiti saranno elencati secondo un ordine. Ogni requisito\ped{\emph{G}} seguirà la seguente codifica: \\
			\begin{center}
			...
			I requisiti saranno inseriti in una tabella, che includerà anche un nome per ogni attributo, una fonte
			 e una breve descrizione. La forma tabellare di un requisito\ped{\emph{G}} risulta quindi essere:
			\begin{table}[H]
        ...
        \begin{itemize}
        	\item Inserire un requisito\ped{\emph{G}} tramite interfaccia web
        ...
        \subsubsection{Tracciamento dei requisiti}
            ...
            \\ \\
            Il tool individua automaticamente le dipendenze di tipo padre-figlio e genera in tal modo un albero dei requisiti navigabile semplicemente. Sono stati definiti dei trigger SQL per cui non risulta possibile segnare come completato un requisito\ped{\emph{G}} che presenta requisiti figli incompleti. Inoltre il completamento di tutti i requisiti figli rende automatico il completamento del padre.
            \\ \\
		...
		nella loro forma di dichiarazione, con un puntatore al "nuovo" requisito\ped{\emph{G}}, che invece avrà le specifiche aggiornate. È compito del
		\emph{Responsabile di progetto} mantenere aggiornata la tabella dei requisiti, copiando prima il requisito\ped{\emph{G}} che necessita di cambiamento
		nella tabella dei vecchi requisiti, e poi aggiornando il requisito\ped{\emph{G}} stesso.
		\\ \\
		...
		Nello specifico i diagrammi UML che il gruppo dovrà utilizzare in fase\ped{\emph{G}} di progettazione sono i seguenti:
		\begin{itemize}
		...
		In linea con quanto definito per la stesura dei diagrammi dei casi d'uso\ped{\emph{G}}, i diagrammi delle componenti dovranno essere realizzati utilizzando il software \emph{Astah Professional} alla versione 7.0 .
		...
		I membri del gruppo sono invitati ad utilizzare design pattern\ped{\emph{G}} noti dove possibile in fase\ped{\emph{G}} di progettazione. L'utilizzo di essi permetterà di avere un'architettura più comprensibile ed estendibile sia dal punto di vista del codice che dal punto di vista della documentazione che lo accompagna. \\ \\
		Ogni volta che una parte dell'architettura del sistema si basa su un design pattern\ped{\emph{G}} sarà necessario fornire una descrizione dello stesso in un'apposita sezione del documento di \emph{Specifica Tecnica}.		
		...
		definite dalla documentazione ufficiale di \emph{Scala\ped{\emph{G}}}: \url{http://docs.scala-lang.org/style/}
		\begin{figure}[H]
				\centering
	...
	\section{Processi di supporto}
		...
		modalità di redazione, verifica\ped{\emph{G}} e approvazione dei documenti.
		...
		è quindi variabile, in una fase\ped{\emph{G}} ci possono essere 2 persone per la scrittura del documento, in quella successiva 3, o solamente una.
		...
		\begin{itemize}
			...
			\end{itemize}
			\item \textbf{Date:} le date presenti nei documenti seguono lo standard ISO 8601:2004\ped{\emph{G}}:
			\begin{center}
				AAAA - MM - GG
			\end{center}
			Dove:
		...
		Per risultare approvato un documento deve aver completato con successo il percorso di verifica\ped{\emph{G}} e validazione
		...
		\begin{itemize}
			...
			\item Y: viene incrementato quando il documento entra in una fase\ped{\emph{G}} successiva del suo ciclo di vita;
		...
		\begin{itemize}
			\item \textbf{Lavorazione/Modifica:} il documento entra in questa fase\ped{\emph{G}} al momento della sua creazione e vi
			rimane per tutto il tempo in cui il suo contenuto viene modificato. Prima di terminare la sua fase\ped{\emph{G}} di modifica ed essere
			...
			\end{figure}
			\item \textbf{Verifica:} quando termina la fase\ped{\emph{G}} di modifica, il documento passa nelle mani dei
		...
		\end{itemize}
		Ogni documento prodotto può attraversare più volte ogni fase\ped{\emph{G}} del suo ciclo di vita, allo stesso modo può non
		...
		\subsection{Gestione delle versioni}
			...
			alla prima, in quanto sono segnate le sezioni che sono state aggiunte o che hanno subito dei cambiamenti dall'ultima
			 verifica\ped{\emph{G}}, evitando così di controllare sempre tutto il documento ad ogni rilascio di versione, per cui ottimizzando 
			...
			di verifica\ped{\emph{G}}, descritte nella sezione Analisi del \emph{Piano di Qualifica v3.0.0}, a causa dei diversi output ottenuti:

			\begin{itemize}
				\item \textbf{Scelta ed approccio al capitolato:} Si devono eseguire le attività di verifica\ped{\emph{G}} sui processi e sui documenti.
				
				\item \textbf{Analisi di dettaglio:} Si devono eseguire le attività di verifica\ped{\emph{G}} sui processi e sui documenti.
				
				\item \textbf{Progettazione e sviluppo:} Si devono eseguire le attività di verifica\ped{\emph{G}} sui processi, sui documenti e sul codice prodotto.
				
				\item \textbf{Sviluppo ulteriore ed incremento\ped{\emph{G}}:} Si devono eseguire le attività di verifica\ped{\emph{G}} sui processi, sui documenti e sul codice prodotto.
				
				\item \textbf{Progettazione e sviluppo:} Si devono eseguire le attività di verifica\ped{\emph{G}} sui processi, sui manuali e sul prodotto finito.
			\end{itemize}
		...
		\subsubsection{Procedure di controllo qualità per i processi}
			...
			\begin{itemize}
				...
				quindi con strumenti automatici ovunque possibile. Alla fine di ogni fase\ped{\emph{G}} di progetto è necessario calcolare
		...
		\subsubsection{Procedure di controllo qualità per i documenti}
			...
			\begin{itemize}
				\item \textbf{Verifica:} le attività di verifica\ped{\emph{G}} sui documenti non devono mai essere trascurate. Nelle prime fasi 
				del progetto i verificatori dovranno sottoporre i documenti ad un'analisi di tipo \emph{Walkthrough}, per passare 
				poi quando possibile all'\emph{Inspection}. Inoltre al termine di ogni fase\ped{\emph{G}} è necessario verificare che le 
		...
		\subsubsection{Procedure di controllo qualità per il prodotto}
			...
			\begin{itemize}
				...
				dei test di validazione\ped{\emph{G}} opportunamente prodotti.
				\item \textbf{Quality Assurance:} il rispetto delle norme di progetto assicura il raggiungimento degli obiettivi 
				di qualità prefissati, riducendo il ricorso a tecniche retrospettive, e con esse si riducono le iterazioni.
			\end{itemize}
		...
		%affidarsi ad il servizio\ped{\emph{G}} di issue tracking \emph{YouTrack} offerto da \emph{JetBrains}. \\
		%Per i progetti fino a dieci utenti, il servizio\ped{\emph{G}} offre 10GB di spazio sul cloud dell'azienda in maniera gratuita.
			...
			\begin{itemize}
				...
				\item Appartenenza al range di accettabilità per tutte le metriche descritte nella sezione Visione generale della strategia di verifica\ped{\emph{G}} del\emph{Piano di Qualifica v3.0.0};
	...
	\subsubsection{Documentazione di pianificazione}
		...
		\begin{itemize}
			...
			\item Open-source;
			\item Genera automaticamente diagrammi di Gantt, WBS\ped{\emph{G}} e PERT;
	...
	\begin{itemize}
		...
		\item Include un ottimo Task\ped{\emph{G}} management (priorità, task history, possibilità di aggiungere in automatico task ricorrenti);
	...
	Come task\ped{\emph{G}} management \emph{Zoho} offre solamente delle To-do List, mentre \emph{Teamwork} ha anche le seguenti
	feature:
	\begin{itemize}
		\item Add Recurring Tasks;
		\item Group Tasks by Projects;
		\item Set Priorities;
		\item Task\ped{\emph{G}} History.
	\end{itemize}
    ...
    backup della \emph{Baseline\ped{\emph{G}}} a cui fa riferimento.
	...
	\subsubsection{Ruoli}
		...
		contemporaneamente più ruoli, la rotazione di altri ruoli come \emph{Analista}, \emph{Progettista} e \emph{Programmatore\ped{\emph{G}}} potrà avvenire
		...
		conclusione.
\\ \\
		Il gruppo stabilisce la rotazione dei ruoli in base alle attività, ai task\ped{\emph{G}}, e alla disponibilità fornita da ogni membro.
\\ \\
	...
	\begin{itemize}
		...
		La scelta di \emph{Telegram} è dovuta alla possibilità di utilizzare il servizio\ped{\emph{G}} sia da desktop che da mobile, e alla
...
\\ \\
		...
		committente può verificarsi un cambiamento riguardante ad un requisito\ped{\emph{G}}, in questo caso il cambiamento va inserito nel
		...
		Durante un \emph{Brainstorming\ped{\emph{G}}} ci sarà un membro con il compito di scrivere le \emph{Minute}, ovvero un \emph{Notaio}.
		Ad ogni \emph{Brainstorming\ped{\emph{G}}} sarà anche scelto un \emph{Moderatore} che ricoprirà un ruolo di servizio\ped{\emph{G}}. Ovvero dovrà far
		rispettare le regole di base. Una volta finito il \emph{Brainstorming\ped{\emph{G}}}, il \emph{Notaio} dovrà riorganizzare gli appunti
		presi in un verbale ordinato.
		
		\subsection{Meccanismi di controllo e rendicontazione}
		
			\subsubsection{Meccanismi di controllo}
				All'interno dell'ambiente di lavoro\ped{\emph{G}} sono stati predisposti meccanismi per:
				\begin{itemize}
			...
			\subsubsection{Andamento delle attività}
				...
				\end{figure}
				Inoltre per ogni attività è predisposta anche una \emph{due to date}, ovvero la data entro la quale la task\ped{\emph{G}} deve 
				...
				task\ped{\emph{G}} in ritardo e provvedere a comunicare con il/i componenti del gruppo a cui essa è assegnata per capire 
