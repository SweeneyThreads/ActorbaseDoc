...
\begin{document}
	...
	\subsection{Scopo del documento}
		...
		Verranno presentati l'architettura, le componenti, le classi e i design pattern\ped{\emph{G}} utilizzati.
	...
	\subsection{Scala}
		Le possibili scelte dettate dal capitolato sono Java e Scala\ped{\emph{G}}. Si è scelto di utilizzare Scala perché offre i seguenti vantaggi:
		\begin{itemize}
            \item \textbf{Completamente Object-Oriented:} A differenza di Java, Scala\ped{\emph{G}} è completamente orientato agli oggetti. Non c'è distinzione del tipo: oggetto - tipo primitivo, ogni valore è semplicemente un oggetto.
			\item \textbf{Staticamente tipato:} È un linguaggio tipato staticamente, questo permette di effettuare più facilmente i test. Inoltre Scala\ped{\emph{G}} è in grado di stabilire il tipo di un oggetto per inferenza.
            \item \textbf{Può eseguire codice Java:} Scala\ped{\emph{G}} può eseguire codice scritto in Java. È dunque possibile utilizzare classi e librerie scritte in Java all'interno di programmi scritti in Scala. 
			...
			\item \textbf{Supporto alla definizione di DSL:} Scala\ped{\emph{G}} supporta nativamente la definizione di DSL.
            \item \textbf{Supporto di Akka\ped{\emph{G}}:} Il linguaggio supporta la libreria Akka che è richiesta dal capitolato.
		\item \textbf{Preferenze del Committente:} Inoltre il Committente ha espresso esplicitamente la sua preferenza sull'utilizzo di Scala\ped{\emph{G}}.
		\end{itemize}
		Problematiche di scala\ped{\emph{G}}:
		\begin{itemize}
		...
		\end{figure}	
	\subsection{Akka}
		L'utilizzo della libreria Akka\ped{\emph{G}} oltre ad essere reso obbligatorio dal committente, fornisce un'eccellente base su cui sviluppare un sistema basato sul modello ad attori.
        Akka\ped{\emph{G}} permette di costruire facilmente applicazioni message-driven che siano estremamente concorrenti, distribuite e resilienti.         
        La natura distribuita e asincrona degli attori messi a disposizione da Akka\ped{\emph{G}} soddisfa pienamente i bisogni del sistema da implementare.
        Akka\ped{\emph{G}} esegue l'ovveride di molti operatori rendendo difficile la comprensione agli utenti poco esperti. 
	\begin{figure} [H]
			\centering
	...
	\section{Descrizione dell'architettura}
		\subsection{Metodo e formalismo di specifica}
			...
			Infine si illustreranno degli esempi di utilizzo dei design pattern\ped{\emph{G}} nell'architettura del sistema.
        ...
        \subsubsection{Server}
			...
			\\
			\noindent La classe \textbf{Server} è l'entry point del programma e gestisce la configurazione\ped{\emph{G}} iniziale all'avvio dello stesso.  
        ...
        \subsubsection{Driver}
        	...
        	Il Driver\ped{\emph{G}} è una libreria, invocando i metodi della quale è possibile effettuare richieste TCP sul socket su cui il
			Server è in ascolto. \\
	...
	\section{Componenti e Classi}
		...
		\subsection{Actorbase.server.Server}
			\subsubsection{Descrizione}
				Classe principale della parte \textbf{Server} del programma. È di fatto l'entry point dello stesso, gestisce la configurazione\ped{\emph{G}} 
				iniziale e avvia il sistema. Utilizza il design pattern\ped{\emph{G}} \textbf{Singleton}.
		...
		\subsection{Actorbase.server.utils.FileManager}
			...
			\subsubsection{Descrizione}
				Classe che fornisce i metodi per leggere i file di configurazione\ped{\emph{G}} di \emph{Actorbase}.
		...
		\subsection{Actorbase.server.utils.Helper}
			...
			\subsubsection{Utilizzo}
				Viene utilizzata dall'attore\ped{\emph{G}} Main\ped{\emph{G}} per recuperare la lista di comandi con la loro spiegazione, è anche possibile recuperare 
		...
		\subsection{Actorbase.server.actors.Doorkeeper}
			\subsubsection{Descrizione}
				Questo attore\ped{\emph{G}} apre il socket su cui i client potranno connettersi, crea un attore\ped{\emph{G}} \textbf{Usermanager} per ogni nuova 
		...
		\subsection{Actorbase.server.actors.Usermanager}
			\subsubsection{Descrizione}
				Questo attore\ped{\emph{G}} si preoccupa di trasformare le stringhe ricevute dal client in messaggi interpretabili da altri attori, e viceversa.
				
			\subsubsection{Utilizzo}
				Viene utilizzato per gestire una connessione da parte di un client, crea un attore\ped{\emph{G}} \textbf{Main} e invia le richieste 
				ricevute all'attore\ped{\emph{G}} creato. 
		...
		\subsection{Actorbase.server.actors.Main}
			\subsubsection{Descrizione}
				Questo attore\ped{\emph{G}} è responsabile del controllo dei permessi delle richieste ricevute. Gestisce tutti i messaggi, elaborandoli o inoltrandoli 
		...
		\subsection{Actorbase.server.actors.MapManager}
			\subsubsection{Descrizione}
				Questo attore\ped{\emph{G}} rappresenta un database di \emph{Actorbase}. Si occupa di indicizzare gli attori che rappresentano le mappe che compongono tale database.
				
			\subsubsection{Utilizzo}
				Viene utilizzato per rappresentare un database, l'attore\ped{\emph{G}} \textbf{Main} lo utilizza per instradare richieste verso uno specifico database. All'avvio il Server crea un \textbf{MapManager} per ogni database presente su disco.
			...
			\subsubsection{Descrizione}
				Questo attore\ped{\emph{G}} rappresenta una mappa di \emph{Actorbase}. Si occupa di indicizzare gli attori che rappresentano le porzioni di dati di tale mappa, sia su RAM che su disco.
		...
		\subsection{Actorbase.server.actors.Storemanager}
			\subsubsection{Descrizione}
				Questo attore\ped{\emph{G}} rappresenta un una porzione di mappa in memoria principale, ha più comportamenti: sia come indicizzazione dei dati che come mantenimento di essi. 
		...
		\subsection{Actorbase.server.actors.Warehouseman}
			\subsubsection{Descrizione}
				Questo attore\ped{\emph{G}} permette l'interazione del database con il filesystem.
			...
			\subsubsection{Descrizione}
				Interfaccia che definisce un attore\ped{\emph{G}} che interagisce con altri nodi del cluster.
		...
		\subsection{Actorbase.server.actors.ReplyActor}
			\subsubsection{Descrizione}
				Interfaccia che definisce un attore\ped{\emph{G}} che risponde con informazioni definite in \emph{Actorbase}.
		...
		\subsection{Actorbase.server.enums.EnumPermissions.Permission}
			...
			\subsubsection{Utilizzo}
				Viene utilizzata dall'attore\ped{\emph{G}} \textbf{Main} per effettuare i controlli dei permessi dei messaggi. 				
		...
		\subsection{Actorbase.server.enums.EnumStoremanagerTypes.StorefinderType}
			\subsubsection{Descrizione}
				Questa classe il comportamento Storefinder\ped{\emph{G}} di un attore\ped{\emph{G}} di tipo \textbf{Storemanager}.
				
			\subsubsection{Utilizzo}
				Viene utilizzata per definire il comportamento Storefinder\ped{\emph{G}}.
			...
			\subsubsection{Descrizione}
				Questa classe il comportamento Storekeeper\ped{\emph{G}} di un attore\ped{\emph{G}} di tipo \textbf{Storemanager}.
				
			\subsubsection{Utilizzo}
				Viene utilizzata per definire il comportamento Storekeeper\ped{\emph{G}}.
			...
			\subsubsection{Descrizione}
				Questa classe il comportamento StorefinderNinja di un attore\ped{\emph{G}} di tipo \textbf{Storemanager}.
			...
			\subsubsection{Descrizione}
				Questa classe il comportamento StorekeeperNinja di un attore\ped{\emph{G}} di tipo \textbf{Storemanager}.
...
\subsection{Actorbase.server.messages.internal.ScalabilityMessages}		
		...
		\subsection{Actorbase.server.messages.internal.ScalabilityMessages.SendMapMessage}
			...
			\subsubsection{Utilizzo}
				Viene inviato da un per mandare una mappa (più item) da un attore\ped{\emph{G}} ad un altro.
		...
		\subsection{Actorbase.server.messages.internal.LinkMessages.LinkMessage}
			\subsubsection{Descrizione}
				Trait per la richiesta di aggiunta o rimozione di un attore\ped{\emph{G}} da parte di un altro attore.
...
\subsection{Actorbase.server.messages.query.admin.SettingMessages.RefreshSettingsInfo}
		...
		\subsection{Actorbase.server.messages.query.user.RowMessages.FindRowMessage}
			...
			\subsubsection{Utilizzo}
				Viene utilizzato per richiedere una riga contenuta in un attore\ped{\emph{G}} di tipo \textbf{Storemanager}.
		...
		\subsection{Actorbase.server.messages.query.user.DatabaseMessages.CreateDatabaseMessage}
			...
			\subsubsection{Utilizzo}
				Viene utilizzato per richiedere la creazione di un nuovo database ovvero di un attore\ped{\emph{G}} \textbf{MapManager}.
		...
		\subsection{Actorbase.server.messages.query.user.DatabaseMessages.ListDatabaseMessage}
			...
			\subsubsection{Utilizzo}
				Viene utilizzato dall'attore\ped{\emph{G}} \textbf{Main} per ottenere la lista di database (\textbf{MapManager}) presenti sul server.
		...
		\subsection{Actorbase.server.messages.query.user.HelpMessages.CompleteHelp}
			...
			\subsubsection{Utilizzo}
				Effettua il percorso descritto in \hyperref[QueryMessage]{Actorbase.server.messages.query.QueryMessage} fermandosi all'attore\ped{\emph{G}} \textbf{Main}.
		...
		\subsection{Actorbase.client}
			...
			\subsubsection{Descrizione}
				...
				È composto due classi che implementano il design pattern\ped{\emph{G}} \textbf{Singleton}.
		...
		\subsection{Actorbase.client.Client}
			\subsubsection{Descrizione}
				...
				Questa classe implementa il design pattern\ped{\emph{G}} \textbf{Singleton}.
				
			\subsubsection{Utilizzo}
				Viene utilizzata per distinguere i comandi di connessione, disconnessione e altri comandi possibili. Inoltre utilizza il driver\ped{\emph{G}} per comunicare 
				con il server.
		...
		\subsection{Actorbase.driver}
			...
			\end{figure}
			\subsubsection{Descrizione}
				Package per la componente driver\ped{\emph{G}} del sistema. 
				È composto una classe che implementa il design pattern\ped{\emph{G}} \textbf{Singleton}, un trait e una sua implementazione.
		...
		\subsection{Actorbase.driver.Driver}
			\subsubsection{Descrizione}
				Classe che implementa il design pattern\ped{\emph{G}} \textbf{Singleton}, 
...
In questa sezione viene illustrata ad alto livello l'architettura della logica ad attori del sistema lato server, vero nucleo di \emph{Actorbase}.
	\subsection{Struttura principale}
				...
				\end{figure}
		La struttura del server può individuare negli attori TCP (forniti da Akka\ped{\emph{G}}) e negli attori \textbf{Doorkeeper} i nodi iniziali. Un attore\ped{\emph{G}} \textbf{Doorkeeper} apre un socket TCP e si mette in ascolto, quando viene stabilità una nuova connessione esso crea un attore\ped{\emph{G}} di tipo \textbf{Usermanager} a cui ne delega la gestione.
		\\ \\
		Dunque ogni client connesso al server è rappresentato e gestito da un attore\ped{\emph{G}} \textbf{Usermanager} dedicato che si occupa di gestire i messaggi inviati dal client, trasformandoli in messaggi gestibili dal sistema ad attori. Tali messaggi vengono prima di tutto gestiti da un attore\ped{\emph{G}} di tipo \textbf{Main}. Esso effettua una prima analisi e gestione dei messaggi. Una volta che un messaggio arriva all'attore \textbf{Main}, infatti, viene interpretato e può essere gestito direttamente dall'attore o inoltrato alle componenti più basse del sistema.
	...
	\subsection{Struttura di un database}
				...
				\end{figure}
Un singolo database del sistema è rappresentato da un attore\ped{\emph{G}} \textbf{MapManager}. Esso si occupa di gestire e indicizzare tutti gli attori di tipo \textbf{IndexManager}, rappresentanti le mappe che compongono tale database.
\\ \\
Ogni \textbf{IndexManager} indicizza e gestisce i dati che compongono la mappa in due modi: in memoria principale e su disco. La gestione in RAM avviene tramite l'utilizzo di attori di tipo \textbf{Storemanager} mentre della gestione dei dati su disco si occupano attori di tipo \textbf{Warehouseman}.
	...
	\subsection{Struttura di una mappa in memoria principale}
				...
				\end{figure}
Una mappa in memoria principale è rappresentata da un albero binario di attori \textbf{Storemanager}. Attori di tale tipo assumono un comportamento diverso nel caso in cui agiscano da indicizzatori di altri \textbf{Storemanager} o da contenitori dei dati veri e propri. Nel primo caso il comportamento adottato è quello dell'attore\ped{\emph{G}} \textbf{Storefinder} (in rosso nel diagramma), nel secondo caso quello dell'attore\ped{\emph{G}} \textbf{Storekeeper} (in blu nel diagramma).
\\ \\
Questa struttura permette di avere un sistema stabile e affidabile. Quando un attore\ped{\emph{G}} raggiunge la sua capienza massima non si divide in due ma genera due figli e prosegue nella gestione dei messaggi che aveva in mailbox. In questo modo il padre non si accorge dello sdoppiamento del figlio, ne del suo cambio di comportamento. D'altro lato il figlio non rischia di perdere messaggi nell'operazione di sdoppiamento.
...
Ogni \textbf{Storefinder} e \textbf{Storekeeper} possiede un insieme di attori \textbf{Storemanager} che agiscono da copia delle proprie informazioni, il comportamento di tali attori è detto \textbf{Ninja}. Essi permettono di avere delle copie di backup che possono essere sostituite agli attori principali in caso di necessità. Inoltre possono essere usati per incrementare le performance del sistema delegando a loro la risoluzione di alcune query, alleggerendo così l'attore\ped{\emph{G}} principale.
	...
	Segue, per ogni Design Pattern\ped{\emph{G}} utilizzato, la descrizione dello scopo, motivazione e applicabilità.
	...
	\subsubsection{Singleton}
				\begin{figure}[H]
					\centering
					\includegraphics[scale = 0.8]{immagini/ST/schemaSingleton.png}
