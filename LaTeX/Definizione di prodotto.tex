...
\begin{document}
	...
	\subsection{Actorbase.server.utils.ServerDependencyInjector (trait)}
		\begin{figure}[H]
			\centering
			\includegraphics[scale=0.6]{Server/ServerDependencyInjectorTrait.jpg}
			\caption{Trait Actorbase.server.utils.ServerDependencyInjector}
		\end{figure}
		\textbf{Descrizione}
			\\ \\
		...
		\textbf{Metodi}
			\\ \\
			\texttt{getStoremanagers(): ConcurrentHashMap}
			\\ \\
			Metodo che permette di ottenere la lista di Usermanager (contratto).
			\\ \\
			Lista parametri del metodo:
			\\ \\
			Nessuno.
	...
	\subsection{Actorbase.server.utils.StandardServerInjector}
		\begin{figure}[H]
			\centering
			\includegraphics[scale=0.6]{Server/StandardServerInjectorClass.jpg}
			\caption{Trait Actorbase.server.utils.StandardServerInjector}
		\end{figure}
		\textbf{Descrizione}
			\\ \\
		...
		\textbf{Utilizzo}
			\\ \\
			Viene utilizzato per delegare la chiamata ai metodi del Server.
			\\ \\
		\textbf{Classi ereditate}
			\begin{itemize}
		...
		\textbf{Metodi}
			...
			\texttt{getStoremanagers(): ConcurrentHashMap}
			\\ \\
			Metodo che permette di ottenere la lista di Usermanager (implementazione).
			\\ \\
			Lista parametri del metodo:
			\\ \\
			Nessuno.
