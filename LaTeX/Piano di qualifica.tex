...
\begin{document}
    ...
    \section{Introduzione}
		\subsection{Scopo del documento}
			...
			processo di verifica\ped{\emph{G}} continuo sulle attività svolte in modo da poter rilevare ed eventualmente 
		...
		\subsection{Riferimenti}
			...
			\subsubsection{Informativi}
				...
				\end{itemize}
	\section{Visione generale della strategia di verifica\ped{\emph{G}}}
...
\subsection{Misure e metriche}
			 Il processo di verifica\ped{\emph{G}}, per avere un valore informativo, deve essere quantificabile e le misure rilevate
 ...
 seguendo il modello incrementale esposto nel \emph{Piano di progetto} si potrà migliorarne la precisione e l'accuratezza.
			...
			\begin{itemize}
				...
				ma fortemente consigliati. Misurazioni al di fuori di questi valori necessitano una verifica\ped{\emph{G}} approfondita e nel
			...
			\subsubsection{Metriche per i processi}
				...
				\begin{itemize}
					\item \textbf{SV (Schedule Variance):} É un indicatore di efficacia\ped{\emph{G}}, mostra se si è o meno in linea con la
					pianificazione temporale rispetto alle attività nella baseline\ped{\emph{G}}. Una schedule variance positiva indica che il
			...
			\subsubsection{Metriche per i documenti}
				...
				\end{itemize}
				\item \textbf{Numero parole:} Questa metrica servirà principalmente per misurare l'incremento\ped{\emph{G}} del documento durante il completamento delle varie fasi.
			...
			\subsubsection{Metriche per il software}
				...
				realmente la fase\ped{\emph{G}} di programmazione.
		...
		\subsection{Strumenti e metodi per l'applicazione delle metriche}
			Le risorse software che si utilizzeranno durante il processo di verifica\ped{\emph{G}} sono:
			\begin{itemize}
				...
				sorgente in vari linguaggi, tra cui \emph{Scala\ped{\emph{G}}}, servirà a calcolare le linee di codice per linee di commento (\url{cloc.sourceforge.net});
				\item \textbf{\emph{ScalaTest}:} framework per i test su Scala\ped{\emph{G}} (\url{http://www.scalatest.org/}).
		...
		\subsection{Metodi}
			...
			\subsubsection{Analisi dei processi}
				I processi compiuti durante una fase\ped{\emph{G}} verranno analizzati secondo il seguente protocollo:
				\begin{itemize}
					\item \textbf{Controllo delle metriche:}
						Alla conclusione di ogni fase\ped{\emph{G}} del progetto si calcolano gli indici definiti nella sezione 2.2.1 confrontando 
					...
					\item \textbf{Analisi PDCA:}
						Secondo il ciclo PDCA una fase\ped{\emph{G}} di progetto ha inizio con la pianificazione di come possono essere migliorati i processi. 
						Durante la fase\ped{\emph{G}} vengono attuati i cambiamenti prefissati e alla fine viene effettuato un controllo. Se un processo 
		...
		tipo possono essere dovuti a sviste o errori di copia nella stesura.
					...
					di questa verifica\ped{\emph{G}} sono automatizzabili, i Verificatori dovranno quindi usare tool ove possibile. 
		...
		\subsection{Organizzazione}
			Il processo di verifica\ped{\emph{G}} inizia fin dal primo rilascio ufficiale nella repository di un documento vX.0.0, il risultato 
			del processo di verifica\ped{\emph{G}} cambia la versione del documento, incrementandone la versione al secondo livello di profondità, 
			il documento quindi passa a vX.1.0. Ogni modifica apportata al documento ne aumenta la versione all'ultimo livello di 
			profondità. Una volta completate le modifiche e le verifiche continue sui documenti, esso passa nella fase\ped{\emph{G}} di validazione\ped{\emph{G}},
			...
			alla prima, in quanto sono segnate le sezioni che sono state aggiunte o che hanno subito dei cambiamenti dall'ultima
			 verifica\ped{\emph{G}}, evitando cosi di controllare sempre tutto il documento ad ogni rilascio di versione, per cui ottimizzando 
			...
			di verifica\ped{\emph{G}} a causa dei diversi output ottenuti:

			\begin{itemize}
				\item \textbf{Analisi:} Si devono seguire le attività di verifica\ped{\emph{G}} descritte nella sezione 2.9.2 e 2.9.1 sui processi 
				e sui documenti attuati.
			\end{itemize}
			
			Come detto in precedenza conclusa la verifica\ped{\emph{G}}, ha inizio il processo di approvazione. Durante questo processo è compito 
		...
		\subsection{Pianificazione strategica e temporale}
			...
			di verifica\ped{\emph{G}}, sia del codice che della documentazione, sia sistematica e ben organizzata; in questo modo 
			l'individuazione, e quindi la correzione degli errori avverrà il prima possibile, limitando la diffusione 
			degli stessi.\\
			Per cercare di ridurre il numero degli errori, e quindi semplificare l'attività di verifica\ped{\emph{G}}, ogni fase\ped{\emph{G}} 
			di codifica o documentazione sarà preceduta da una fase\ped{\emph{G}} di studio preliminare. Evitando le imprecisioni 
		...
		\subsection{Responsabilità}
			...
			\begin{itemize}
				\item Accertarsi che le attività di verifica\ped{\emph{G}} vengano svolte sistematicamente secondo quanto 
			...
			\begin{itemize}
				\item Effettuare la verifica\ped{\emph{G}} dei documenti con strumenti e metodi proposti nel \emph{Piano di Qualifica};
		...
		\subsection{Risorse}
			...
			\subsubsection{Risorse umane}
				...
				\begin{itemize}
					...
					\item \emph{Programmatore\ped{\emph{G}}};
			...
			\subsubsection{Risorse software}
				...
				\begin{itemize}
					...
					\item allo sviluppo di codice \emph{Scala\ped{\emph{G}}};
		...
		\subsection{Analisi}
			\subsubsection{Tecniche per l'analisi statica}
				...
				\subsubsection{Walkthrough}
					...
					Dopo una fase\ped{\emph{G}} iniziale in cui i Verificatori leggono il documento ed individuano potenziali errori essi 
			...
			\subsubsection{Tecniche per l'analisi dinamica}
				...
				L'attore\ped{\emph{G}} che esegue un test deve definire a priori ed avere il pieno controllo su:
				\begin{itemize}
					...
					\item \textbf{Procedure:} descrizione delle azioni compiute dall'attore\ped{\emph{G}} (umano o computer che sia) 
				...
				\begin{itemize}
					...
					unità che compongono l'intero sistema. Essi vengono testati attraverso l'uso di stub\ped{\emph{G}}, driver\ped{\emph{G}} e logger. Queste
					...
					\item \textbf{Test di integrazione:} Consiste nella verifica\ped{\emph{G}} di componenti del sistema che vengono aggiunti
					...
					\item \textbf{Test di Sistema:} Consiste nella validazione\ped{\emph{G}} del prodotto software\ped{\emph{G}} una volta che siano stati
					aggiunti tutti i componenti e lo si ritiene giunto ad una versione definitiva. Lo scopo principale è verificare
					che ci sia totale copertura dei requisiti stabiliti nella fase\ped{\emph{G}} di Analisi di dettaglio. È obiettivo
	...
	\section{Gestione amministrativa della revisione}
		...
		\subsection{Procedure di controllo qualità per il prodotto}
			...
			\begin{itemize}
				...
				\item \textbf{Verifica:} determina che l'output di una fase\ped{\emph{G}} sia consistente, completo e corretto. 
...
Riportiamo gli standard di riferimento per sviluppare le metriche e i metodi atti a garantire la qualità del prodotto.
			...
			\subsubsection{Qualità di processo}
				...
				\begin{itemize}
					...
					\item \textbf{Do:} fase\ped{\emph{G}} di esecuzione delle attività pianificate;
					\item \textbf{Check:} vengono confrontati i dati in uscita dalla fase\ped{\emph{G}} \emph{Do} con quelli 
					pianificati nella fase\ped{\emph{G}} \emph{Plan}, per intervenire in tempo e migliorare i risultati;
					\item \textbf{Act:} fase\ped{\emph{G}} in cui si mette in pratica il miglioramento continuo dei processi
					 utilizzando i risultati della verifica\ped{\emph{G}} per modificare gli aspetti critici dei processi in esame.
				\end{itemize}
				\begin{figure}[H]
					\centering
			...
			\subsubsection{Qualità di prodotto}
				...
				\begin{itemize}
					\item \textbf{Funzionalità:} capacità del prodotto software\ped{\emph{G}} di fornire funzioni che rispondano
					a esigenze stabilite
					\begin{itemize}
						\item \textbf{Idoneità:} capacità del prodotto software\ped{\emph{G}} di fornire un insieme di funzioni per attività
						specifiche già conosciute dall'utente;
						\item \textbf{Accuratezza:} capacità del prodotto software\ped{\emph{G}} di fornire risultati esatti o concordi al grado di precisione necessario;
						\item \textbf{Interoperabilità:} capacità del prodotto software\ped{\emph{G}} di interagire con uno o più sistemi 
						precedentemente specificati;
						\item \textbf{Sicurezza:} capacità del prodotto software\ped{\emph{G}} di proteggere dati e informazioni;
						\item \textbf{Conformità funzionale:} capacità del prodotto software\ped{\emph{G}} di aderire a standard, convenzioni
						o regolamentazioni e prescrizioni in materia di funzionalità.
					\end{itemize}
					
					\item \textbf{Affidabilità:} capacità del prodotto software\ped{\emph{G}} di mantenere uno specifico livello di prestazioni
					quando usato
					\begin{itemize}
						\item \textbf{Maturità:} capacità del prodotto software\ped{\emph{G}} di non fallire a causa di errori nel software;
						\item \textbf{Tolleranza agli errori:} capacità del prodotto software\ped{\emph{G}} di mantenere un adeguato livello di
						prestazioni e funzioni in caso di errori software o di violazioni;
						\item \textbf{Capacità di recupero:} capacità del prodotto software\ped{\emph{G}} di ristabilire un adeguato livello di
						performance e di recuperare i dati in caso di errori;
						\item \textbf{Conformità di affidabilità:} capacità del prodotto software\ped{\emph{G}} di aderire a standard,
					...
					quando usato
					\begin{itemize}
						\item \textbf{Comprensibilità:} capacità del prodotto software\ped{\emph{G}} di far comprendere all'utente se il
						prodotto è adatto ad uno specifico scopo;
						\item \textbf{Apprendibilità:} capacità del prodotto software\ped{\emph{G}} di ridurre all'utente il tempo necessario
						per apprendere le sue funzioni;
						\item \textbf{Operabilità:} capacità del prodotto software\ped{\emph{G}} di essere utilizzato dall'utente in modo
						controllato
						\item \textbf{Attrattiva:} capacità del prodotto software\ped{\emph{G}} di creare interesse nell'utente;
						\item \textbf{Conformità di usabilità:} capacità del prodotto software\ped{\emph{G}} di aderire a standard, convenzioni
					...
					\begin{itemize}
						...
						\item \textbf{Utilizzo di risorse:} capacità del prodotto software\ped{\emph{G}} di utilizzare quantità e tipo di
						risorse adeguate durante la sua esecuzione;
						\item \textbf{Conformità di efficienza\ped{\emph{G}}:} capacità del prodotto software\ped{\emph{G}} di aderire a standard,
						convenzioni o regolamentazioni in materia di efficienza\ped{\emph{G}}.
					\end{itemize}										
					
					\item \textbf{Manutenibilità:} capacità del prodotto software\ped{\emph{G}} di essere modificato e ampliato.
					\begin{itemize}
						...
						\item \textbf{Modificabilità:} capacità del prodotto software\ped{\emph{G}} di permettere l'implementazione di una
						specifica modifica o di un aggiornamento;
						\item \textbf{Stabilità:} capacità del prodotto software\ped{\emph{G}} di evitare effetti indesiderati causati da uno o
						più aggiornamenti o modifiche;
						\item \textbf{Testabilità:} capacità del prodotto software\ped{\emph{G}} di consentire una facile validazione\ped{\emph{G}} di una
						versione modificata del software;
						\item \textbf{Conformità di manutenibilità:} capacità del prodotto software\ped{\emph{G}} di aderire a standard,
						convenzioni o regolamentazioni in materia di manutenibilità.
					\end{itemize}										
					
					\item \textbf{Portabilità:} capacità del prodotto software\ped{\emph{G}} di poter essere trasferito da un ambiente di lavoro
					ad un altro sia dal punto di vista hardware che per quanto riguarda il sistema operativo
					\begin{itemize}
						\item \textbf{Adattabilità:} capacità del prodotto software\ped{\emph{G}} di essere adattato a diversi ambienti di
						lavoro senza la necessità di effettuare modifiche aggiuntive;
						\item \textbf{Installabilità:} capacità del prodotto software\ped{\emph{G}} di poter essere installato in specifici
						ambienti;
						\item \textbf{Coesistenza:} capacità del prodotto software\ped{\emph{G}} di coesistere in ambienti comuni con altri
						software indipendenti condividendo risorse comuni;
						\item \textbf{Sostituibilità:} capacità del prodotto software\ped{\emph{G}} di poter sostituire un software analogo o
						simile nello stesso ambiente;
						\item \textbf{Conformità di portabilità:} capacità del prodotto software\ped{\emph{G}} di aderire a standard,
				...
				\begin{itemize}
					...
					appropriate in relazione all'efficacia\ped{\emph{G}} ottenuta in contesto d'uso definito. 
				...
				\end{figure}
				\subsection{Resoconto delle attività di verifica\ped{\emph{G}}}
				\subsubsection{Riassunto delle attività di verifica\ped{\emph{G}}}
			Durante la stesura della documentazione sono stati verificati ad ogni modifica sostanziale la documentazione stessa, i
			casi d'uso\ped{\emph{G}} e i requisiti; è stato in oltre verificato se i processi si sono svolti in maniera corretta.
			\subsubsection{Documentazione}
				I documenti sono stati verificati tramite walkthrough da due persone differenti seguendo il seguente protocollo:
				\begin{itemize}
					\item Verifica\ped{\emph{G}} della sintassi e della correzione ortografica
					\item Verifica\ped{\emph{G}} della chiarezza espositiva
					\item Verifica\ped{\emph{G}} del rispetto delle \emph{Norme Di Progetto v1.1.1} capitolo 3.1
					\item Verifica\ped{\emph{G}} dell'uniformità dei termini rispetto allo stesso documento
					\item Verifica\ped{\emph{G}} dell'uniformità dei termini rispetto agli altri documenti
					\item Verifica\ped{\emph{G}} che i termini che lo necessitano siano stati inseriti nel glossario
			...
			\subsubsection{Casi d'uso}
				I casi d'uso\ped{\emph{G}} sono stati verificati sempre tramite walkthrough ponendo l'attenzione sui seguenti punti:
				\begin{itemize}
					...
					2.1.3
					\item Uniformità dei casi d'uso\ped{\emph{G}} rispetto al capitolato
					\item Rispetto della struttura definita nelle \emph{Norme Di Progetto v1.1.1} sezione 2.3.1
				\end{itemize}

				I Casi d'uso\ped{\emph{G}} sono stati successivamente ricontrollati all'interno del documento di \emph{Analisi Dei Requisiti} come
				descritto nella sezione 4.2.2
			\subsubsection{Requisiti}
				I requisiti sono stati controllati tramite walkthrough seguendo il seguente protocollo:
					\begin{itemize}
						\item Verifica\ped{\emph{G}} che la fonte del requisito\ped{\emph{G}} sia corretta.
						\item Nel caso in cui la fonte sia un UC, verifica\ped{\emph{G}} che sia l?UC corretto e valutazione
	 sulla possibilità di dedurre altri requisiti dallo stesso.
						\item Verifica\ped{\emph{G}} uniformità dei termini tra i requisiti.
						\item Rispetto del codice e della struttura definiti nelle \emph{Norme di Progetto v1.1.1} sezione 2.1.2.
						\item I Casi d'uso\ped{\emph{G}} sono stati successivamente ricontrollati all'interno del documento di \emph{Analisi Dei Requisiti} 
			...
			\subsubsection{Processi}
				...
				\begin{itemize}
					\item Che la verifica\ped{\emph{G}} non sia stata effettuata dallo stesso che ha prodotto il materiale da verificare.
					\item Che i documenti siano stati prodotti nell'ordine e nei tempi corretti.
					\item Che le ore di verifica\ped{\emph{G}} siano almeno 30\% delle ore totali.
					\item Che le riunioni e i brainstorming\ped{\emph{G}} si siano svolti come definito nelle \emph{Norme di Progetto v1.1.1} sezione 4.2.3.
				\end{itemize}
				
		
		\subsection{Dettaglio delle verifiche tramite analisi}
			\subsubsection{Processi}
				Vengono qui di seguito riportate le macro-attività della fase\ped{\emph{G}} di analisi con associati i relativi valori di SV e BV.
					\begin{table}[H]
				...
				Durante la prima fase\ped{\emph{G}} di analisi le problematiche riscontrate sui processi sono dovute per la maggior parte 
				all'inesperienza del gruppo. A questa fase\ped{\emph{G}} seguirà una pianificazione su come migliorare i processi che sono 
...
\end{itemize}
	...
	\subsection{Pianificazione dei test}
		Di seguito saranno descritti i test di sistema, integrazione e validazione\ped{\emph{G}} che sono previsti.
		Nelle tabelle sottostanti la sigla N.E sta per non eseguito.				
	\subsubsection{Test di sistema}
		Viene mostrata di seguito la tabella con tutti i test di sistema che consentono di verificare il sistema rispetto ai requisiti descritti nell'\emph{Analisi dei Requisiti v1.3.11} e la tabella che traccia ogni requisito\ped{\emph{G}} al test.
		Per ogni comando testato sarà anche verificato il corretto riconoscimento e la corretta gestione degli errori.
		I test di sistema sono nominati per associazione ai casi d'uso\ped{\emph{G}}.
